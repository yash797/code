%Write a program to plot  signal constellation diagram of received 8-PSK signal 
...
%obj -2
%Write a program to plot Practical and Theoretical BER vs SNR graph of received 8-PSK signal 
in the presence of AWGN for ML receiver.
...
pkg load communications
N = 3000; % Number of bits to be transmitted using *-PSK
% Too large value may slow down the program x = randi([0,1],1,N); % Random input bits generation M = 8; % Number of Symbols in 8-PSK

% Symbol Generation yy = [];
for i=1:3:length(x)
if x(i)==0 && x(i+1)==0 & x(i+2)==0 y = cosd(0)+1j*sind(0);
elseif x(i)==0 && x(i+1)==0 & x(i+2)==1 y = cosd(45)+1j*sind(45);
elseif x(i)==0 && x(i+1)==1 & x(i+2)==1 y = cosd(90)+1j*sind(90);
...
% Transmitted Symbols yy = [yy y];
endfor

% Detection based on euclidean distance ber_simulated = [];
ber_theoretical = [];
...
% Calculation of Euclidian Distances of received symbols from reference symobols min_dist_index = [];
for i=1:length(r) Dist = [];
for k=1:length(ref_symbols)
dist=sqrt((real(r(i))-real(ref_symbols(k)))^2 +(imag(r(i))-imag(ref_symbols(k)))^2); Dist = [Dist dist];
endfor
min_dist_index = [min_dist_index find(Dist==min(Dist))]; endfor

% Estimation of Bits x_estimated = []; for i=1:length(r)
...
x_estimated = [x_estimated 1 0 0]; endif
endfor
...
% BER Plotting
semilogy(EbN0db, ber_simulated, 'ro-', EbN0db, ber_theoretical, 'k>-'); title('BER vs Eb/N0 Plot for 8-PSK');
xlabel('Eb/N0 (dB)');
ylabel('BER');
grid on;
